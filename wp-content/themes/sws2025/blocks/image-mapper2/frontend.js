 ...
 */
    ...
    startOverlayPulse() {
        ...
        if (!this.state.currentContainer) {
            console.log('No currentContainer');
            return;
        }

        // Check if pulse config exists
        if (!this.config.animation.overlayPulse) {
            console.log('No overlayPulse config');
            return;
        }

        // Get all overlays from current container
        const overlays = this.state.currentContainer.querySelectorAll(`${this.config.selectors.overlay} img`);
