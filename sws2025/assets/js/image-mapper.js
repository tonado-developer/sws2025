...
class WordPressImageMapper {
    ...
    hideAllElements(onComplete) {
        ...
        // Collect all visible related elements
    ...
    showElementsForMarker(markerId, onComplete) {
        ...
        this.hideAllElements(() => {
            ...
            elementsToShow.forEach(el => {
                gsap.set(el, { opacity: 0 }); // Reset opacity first
                tl.to(el, {
                    opacity: 1,
    ...
    calculateBoundaryConstraints(parentRect, containerRect, scale, translateX, translateY) {
        ...
        // Calculate parent position after transform
        ...
        const parentOffset = {
            x: (parentRect.left - containerRect.left) * scale,
            y: (parentRect.top - containerRect.top) * scale
        };
        ...
        const parentBounds = {
            left: containerCenter.x + translateX - scaledContainerWidth/2 + parentOffset.x,
            right: 0,
            top: containerCenter.y + translateY - scaledContainerHeight/2 + parentOffset.y,
            bottom: 0
        };
        ...
        if (parentBounds.top > padding.y) {
            translateY -= (parentBounds.top - padding.y);
